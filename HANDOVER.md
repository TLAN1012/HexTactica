...
## 2026-07-06 大改版：全面轉為小隊制單機戰役

專案從「2D6 骰子 + 士氣的歷史戰棋(熱座對戰)」**整組換掉**,改為
**小隊制六角戰棋單機戰役**。玩法致敬經典六角戰棋手遊的小隊制戰鬥,
但規則與程式皆為原創實作(文件不使用任何商標遊戲名稱)。

### 玩法一句話
一隊 = N 名士兵,傷害 = 存活人數 × 單兵傷害;推進 10 關戰役,打贏拿金幣、
補兵、招募、升級,對抗血狼劫掠團。

### 新架構(單一真實來源都在 `src/game/`)
```
src/
├── engine/hex.ts        ← 唯一保留的舊模組(六角幾何)
├── game/                ← 純函式引擎(無 DOM、可測試)
│   ├── types.ts         ← 核心型別
│   ├── units.ts         ← 七兵種資料表
│   ├── terrain.ts       ← 五地形
│   ├── progression.ts   ← 升級成長(XP_THRESHOLDS、statMul…)
│   ├── combat.ts        ← 傷害公式 + 攻擊預覽(與結算共用)
│   ├── battle.ts        ← 戰鬥 reducer(移動/攻擊/反擊/回合)
│   ├── ai.ts            ← 敵方啟發式 AI
│   ├── maps.ts          ← 種子式地圖生成
│   └── campaign.ts      ← 任務表、金幣經濟、名冊、存檔、結算
├── ui/                  ← Title / Campaign / Army / Battle / BattleLog 五畫面
├── App.tsx              ← 畫面路由 + 戰役 state
└── main.tsx
```

### 這次改了什麼
- **玩法**:小隊制傷害池、每回合一次啟動(移動+攻擊)、近戰反擊、
  七兵種特性(衝鋒/先制/克騎/拋射/散兵)、五地形、種子固定地圖。
- **戰役層**:10 關 `MISSIONS`、金幣經濟(招募/補兵/解散,名冊上限 6)、
  擊殺升級(最高 5 級)、localStorage 存檔、首通全額 / 重刷 40% 獎金。
- **移除**:2D6/CRT、士氣與潰敗狀態機、天氣系統、歷史場景 JSON、
  資料驅動 rules/ 目錄、熱座對戰、AudioManager 預留 —— 這些皆為舊版概念。
- **文件**:README(玩家)、RULES(完整規則手冊)、DESIGN(新架構)全部重寫;
  以下舊紀錄保留備查但已過時。
...
# 以下為舊版規則時代的紀錄,已過時(2D6 + 士氣歷史戰棋,僅供歷史備查)
